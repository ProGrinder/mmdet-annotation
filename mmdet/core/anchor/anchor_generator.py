...
class AnchorGenerator:
    ...
    Args:
        ...
        strides: 锚生成器的步幅。这与 FPN 特征图步幅一致
        => feature map的stride其实就是feature map 与 original image的坐标对应关系
        feature map上所有的点都是grid，
        先以左上角为(0,0)原点，生成【base size = feature map的stride】的base anchor(所以越高层的feature map的anchor的base size越大)
        再通过偏移幅度为stride的移位来在每一个grid上生成grid achor
        如果这个grid point映射回上一层feature map是在padding上，就设为valid = fasle
        valid = fasle的grid的上面生成的grid anchor，后续不计算loss
        base_sizes: list[int]，记录每一个level中anchor的基本大小
        octave_base_scale:  特征图 anchor 的基本尺度。值越大，所有 anchor 的尺度都会变大
        scales_per_octave:  特征图的尺度倍数
    ...
    def __init__(self,
        ...
        self.strides = [_pair(stride) for stride in strides]  # _pair()  扩展成两个维度(w_stride, h_stride)
        # 如果base_size没有设置，则base_sizes取(w_stride, h_stride)中的短边
        self.base_sizes = [min(stride) for stride in self.strides
                           ] if base_sizes is None else base_sizes
        # 确保level总层级 = len(base_sizes) = len(strides)
        assert len(self.base_sizes) == len(self.strides), \
            'The number of strides should be the same as base sizes, got ' \
            f'{self.strides} and {self.base_sizes}'

        # calculate scales of anchors  # 检查是否同时设置或同时没设置两种scale
    ...
    # @property 该装饰器会将方法转换为相同名称的只读属性
    @property
    def num_base_anchors(self):
        """list[int]: total number of base anchors in a feature grid"""
        return self.num_base_priors

    # 从这里可以看出num_base_priors和num_base_anchor是相同的属性
    @property
    ...
    def gen_base_anchors(self):
        ...
        # len(base_sizes) = len(stride) = level总层级数
        for i, base_size in enumerate(self.base_sizes):
            center = None
            # self.center为None， 则中心点默认为(0, 0)
            if self.centers is not None:
                center = self.centers[i]
            # 不断添加单层level中的base_anchors
    ...
    def gen_single_level_base_anchors(self,
        ...
        # 开根号是什么意思？ => 根号n : 1 / 根号n == n,保证长宽的比率ratios正确
        h_ratios = torch.sqrt(ratios)
        w_ratios = 1 / h_ratios
        # .view(-1)把数据转换为一维的，并按先后顺序排列
        if self.scale_major:
        ...
        # pixel center
        # 返回base_anchor的左上、右下两点坐标的4个坐标值Tensor:(9,)
        base_anchors = [
            x_center - 0.5 * ws, y_center - 0.5 * hs, x_center + 0.5 * ws,
            y_center + 0.5 * hs
        ]
        # base_anchors: 按-1维度拼接Tensor，变成一个二维Tensor:(9,4)=>left,top,right,bottom
    ...
    def _meshgrid(self, x, y, row_major=True):
        ...
        # 为feature map生成栅状网格(网格栅)
        """
        # use shape instead of len to keep tracing while exporting to onnx
        # tensor.repeat，复制后拼接
        # x.shape[0] = feat_w, y.shape[0] = feat_h
        # xx.shape = tensor([(0, 1, 2, 3,.., feat_w) * stride,(0, 1, 2, 3,.., feat_w) * stride，重复y.shape[0]次])
        # yy.shape = tensor([(0, 0, 0,...,x.shape[0]个0) * stride, (1, 1, 1,...,x.shape[0]个1) * stride,
        # ...(feat_h,feat_h,feat_h...,x.shape[0]个feat_h) * stride])
        # 这样就能得到一张feature map上所有网格点x,y坐标的tenor
    ...
    def single_level_grid_priors(self,
        ...
        # 为每个feature map按自身的stride分配grid(计算grid anchor的中心点的shift_x, shift_y[以整张feature map的左上角为原点])
        #
        """
        ...
        # shifts: 拓展到左上右下两个坐标的偏移 Tensor:(feat_h*feat_w, 4)
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1)
        # first feat_w elements correspond to the first row of shifts
        # add A anchors (1, A, 4) to K shifts (K, 1, 4) to get
        # shifted anchors (K, A, 4), reshape to (K*A, 4)

        # 通过广播运算，在每一个grid上都生成了A个anchor, K = feat_w * feat_h
